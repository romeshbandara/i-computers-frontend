                    ...
                    <table className="w-full min-w-[800px] text-center border-collapse">
                        ...
                        <tbody className="divide-y divide-white/5 text-xs sm:text-sm text-gray-300">
                            {messages.map((item, index) => (
                                ...
                                >
                                    <td className="p-3.5 font-mono text-cyan-400 text-xs">{formatTimestamp(item.time)}</td>
                                    <td className="p-3.5 text-white">{item.name}</td>
                                    <td className="p-3.5 text-white">{item.email}</td>
                                    <td className="p-3.5 font-medium text-white">{item.subject}</td>
                                    <td className="p-3.5 max-w-60 font-medium text-white truncate">{item.message}</td>
                                    <td className="p-3.5">
                                        <div className="flex justify-center items-center">
                                            <MessageModal item={item} refresh={() => setRefreshIndex((prev) => prev + 1)}/>
                                        </div>
                                    </td>
                                </tr>
                            ))}
